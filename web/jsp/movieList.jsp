    ...
    <script>
        ...
        function initMovieList(){
            ...
            if(getUrlParams("type") == "全部" || getUrlParams("type") == null){
                $.ajax({
                    type: "post",
                    url: url + "/movie",
                    data: {method:"findMoviesByType",order: order,type:type,area:area,year:year,page:page,limit:limit,searchMovie:searchMovie},
                    dataType: "json",
        ...
        function initTags(){
            ...
            var TypeActive = [],
                AreaActive = [],
                YearActive = [];
            var urlTemp = ["&type="+type,"&area="+area,"&year="+year,"&searchMovie="+searchMovie];
            ...
            for(var i=0;i<TypeStr.length;i++){
                ...
                tagsType.append(
                    "<li " + TypeActive[i] + ">" +
                        "<a href=\"?"+ urlTemp[0] + urlTemp[1] + urlTemp[2] + urlTemp[3] + "\">"+
                            TypeStr[i] +
                        "</a>" +
                    "</li>"
                );
            }
            urlTemp = ["&type="+type,"&area="+area,"&year="+year,"&searchMovie="+searchMovie];
        ...
        function initParams(){
            ...
            if(getUrlParams('searchMovie') == null){
            	searchMovie = "";
            }else{
            	searchMovie = getUrlParams('searchMovie');
            	$("#searchMovie").val(searchMovie);
            }
        }
        //获取url参数
